...
import { AddressModalbilling } from '../../../checkout/Model';

function CheckoutbillingAddress({ allAddressData, handleClick, handlebackClick }) {
    ...
    const handleCheckboxChange = (id) => { 
        ...
        localStorage.setItem('billAddress', JSON.stringify({ id }));
    };

    useEffect(() => {
        const storedAddress = JSON.parse(localStorage.getItem('billAddress'));
        if (storedAddress) {
                ...
                {allAddressData?.map((item, index) => (
                    <div className='lg:col-span-6 col-span-12 shadow-md rounded-lg p-4 border-[#00A762] border-2' key={index}>
                        ...
                        <div className="flex w-full items-center justify-between">
                            {item?.default_billing_address === true ? (
